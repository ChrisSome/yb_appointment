...
$can = Yii::$app->user->can('appointment/domain/index');
$canAdd = Yii::$app->user->can('appointment/domain/create');
$canEdit = Yii::$app->user->can('appointment/domain/update');
$canOper = Yii::$app->user->can('appointment/domain/chg-status');

$this->title = Yii::t('app', 'appointment/domain/index');
?>
                    ...
                    <div class="panel-heading"><strong><span
                        ...
                        <div class="pull-right" style="margin-top:-5px;">
                          <!--  <a type="button" class="btn  btn-sm"
                               href="<?/*= Yii::$app->urlManager->createUrl(array_merge(['/appointment/domain/index'], $params, ['export' => 'excel'])) */?>"><span
                                        class="glyphicon glyphicon-log-out"></span><?/*= Yii::t('app', 'excel') */?>
                            </a>-->
                            <?php if ($canAdd): ?>
                            <a type="button" class="btn  btn-sm"
                               href="<?= Yii::$app->urlManager->createUrl(array_merge(['/appointment/domain/create'])) ?>"><span
                                        class="glyphicon 	glyphicon glyphicon-plus"></span><?= Yii::t('app', 'add') ?>
                            </a>
                                <?php endif; ?>
                                ...
                                if (isset($params['showField']) && $params['showField']) {
                                    ...
                                    foreach ($params['showField'] as $value) {
                                        if (in_array($value, $no_sort_field)) {
                                            ...
                                            array_unshift($newParams, '/appointment/domain/index');
                                ...
                                <tr>
                                    ...
                                    <?php if ($canEdit || $canOper): ?>
                                        <td>
                                            ...
                                            <?php if ($canEdit) echo Html::a(Html::button(Yii::t('app', 'edit'), ['class' => 'btn btn-warning btn-xs']), ['update', 'id' => $one['id']], ['title' => Yii::t('app', 'edit')]) ?>
                                            <?php if ($canOper) echo Html::a(Html::button(Yii::t('app', $one['status'] == '禁用' ? '启用' : '禁用'), ['class' => 'btn btn-danger btn-xs']), ['chg-status', 'id' => $one['id'], 'status' => intval($one['status'] == '禁用' ? 1 : 0)], [
                                                'title' => Yii::t('app', 'User Delete'),
                                                'data' => [
                                                    'method' => 'post',
                                                    'confirm' => $one['status'] == 0 ? '确定启用该域名？' : '确定禁用该域名？'
                                                ],]) ?>
                                        </td>
                                    <?php endif; ?>
                                </tr>
